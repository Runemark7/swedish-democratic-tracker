-- Seed: 27 expenditure areas (utgiftsområden) and national budget data 2020-2025.
-- Amounts in KSEK (thousands of kronor). Source: ESV Statsliggaren (beslutad budget).
-- Idempotent via ON CONFLICT DO NOTHING.

-- ── Expenditure Areas ─────────────────────────────────────────────────

INSERT INTO expenditure_areas (code, name, description, sort_order) VALUES
  ('UO1',  'Rikets styrelse',                                'Kungahuset, riksdagen, regeringskansliet, länsstyrelser',                    1),
  ('UO2',  'Samhällsekonomi och finansförvaltning',          'Finansinspektionen, Riksgälden, statistik',                                  2),
  ('UO3',  'Skatt, tull och exekution',                      'Skatteverket, Tullverket, Kronofogden',                                      3),
  ('UO4',  'Rättsväsendet',                                  'Polisen, domstolar, kriminalvård, åklagare',                                 4),
  ('UO5',  'Internationell samverkan',                       'EU-avgift, utrikesförvaltning',                                              5),
  ('UO6',  'Försvar och samhällets krisberedskap',           'Försvarsmakten, MSB, totalförsvar',                                          6),
  ('UO7',  'Internationellt bistånd',                        'Sida, humanitärt bistånd',                                                   7),
  ('UO8',  'Migration',                                      'Migrationsverket, asylmottagning',                                           8),
  ('UO9',  'Hälsovård, sjukvård och social omsorg',          'Folkhälsomyndigheten, läkemedel, socialtjänst',                              9),
  ('UO10', 'Ekonomisk trygghet vid sjukdom och funktionsnedsättning', 'Sjukpenning, aktivitetsersättning, Försäkringskassan',              10),
  ('UO11', 'Ekonomisk trygghet vid ålderdom',                'Garantipension, äldreförsörjningsstöd',                                      11),
  ('UO12', 'Ekonomisk trygghet för familjer och barn',       'Barnbidrag, föräldraförsäkring, underhållsstöd',                             12),
  ('UO13', 'Jämställdhet och nyanlända invandrares etablering', 'Etableringsersättning, jämställdhetspolitik',                             13),
  ('UO14', 'Arbetsmarknad och arbetsliv',                    'Arbetsförmedlingen, arbetsmarknadspolitik, arbetsmiljö',                     14),
  ('UO15', 'Studiestöd',                                     'CSN, studiemedel, studiebidrag',                                            15),
  ('UO16', 'Utbildning och universitetsforskning',           'Grundskola, gymnasie, högskolor, forskning',                                 16),
  ('UO17', 'Kultur, medier, trossamfund och fritid',         'Kulturrådet, public service, idrott',                                        17),
  ('UO18', 'Samhällsplanering, bostadsförsörjning och byggande samt konsumentpolitik', 'Boverket, bostadsbidrag, Lantmäteriet',           18),
  ('UO19', 'Regional utveckling',                            'Regionala tillväxtmedel, transportbidrag',                                   19),
  ('UO20', 'Allmän miljö- och naturvård',                    'Naturvårdsverket, klimatinvesteringar, kemikalier',                           20),
  ('UO21', 'Energi',                                         'Energimyndigheten, energiforskning, elsäkerhet',                             21),
  ('UO22', 'Kommunikationer',                                'Trafikverket, vägar, järnväg, sjöfart, luftfart',                            22),
  ('UO23', 'Areella näringar, landsbygd och livsmedel',      'Jordbruksstöd, skogsbruk, fiske, djurskydd',                                23),
  ('UO24', 'Näringsliv',                                     'Tillväxtverket, innovationsstöd, handel, turism',                            24),
  ('UO25', 'Allmänna bidrag till kommuner',                   'Kommunalekonomisk utjämning, LSS-kostnadsutjämning',                        25),
  ('UO26', 'Statsskuldsräntor m.m.',                         'Räntor på statsskulden',                                                     26),
  ('UO27', 'Avgiften till Europeiska unionen',               'Sveriges EU-avgift',                                                         27)
ON CONFLICT (code) DO NOTHING;

-- ── Budget Years ──────────────────────────────────────────────────────
-- total_ksek is the sum of all utgiftsområden (beslutad statsbudget).

INSERT INTO budget_years (year, status, total_ksek) VALUES
  (2020, 'decided', 1026700000),
  (2021, 'decided', 1108800000),
  (2022, 'decided', 1105300000),
  (2023, 'decided', 1191400000),
  (2024, 'decided', 1244600000),
  (2025, 'decided', 1297300000)
ON CONFLICT (year, status) DO NOTHING;

-- ── Budget Allocations (KSEK) ─────────────────────────────────────────
-- Each row: (year, area_code, amount_ksek)
-- Data from ESV Statsliggaren / budgetpropositionen (beslutad budget).

-- Helper: insert via subselect to reference budget_years and expenditure_areas by natural keys.
