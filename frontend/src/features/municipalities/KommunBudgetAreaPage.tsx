...
// ── MiniLineChart ─────────────────────────────────────────────────────────────
// Simple SVG polyline for the 4-year trend within a kommun.
...
// ── KommunBudgetAreaPage ──────────────────────────────────────────────────────

export function KommunBudgetAreaPage() {
  ...
  const { data: historySnaps = [], isLoading: loadingHistory } =
    useKommunBudgetHistory(code);
  const { data: areaPoints = [], isLoading: loadingArea } =
    useAreaAcrossMunicipalities(areaName);

  const isLoading = loadingKommuns || loadingHistory || loadingArea;

  // Build kommun code → name map
  const kommunName = kommunList?.find((r) => r.code === code)?.name ?? code;
  const codeToName = new Map(kommunList?.map((r) => [r.code, r.name]) ?? []);

  // ── 4-year trend for this kommun + area ──────────────────────────────────
  const trendPoints: { year: number; value: number }[] = historySnaps
    .filter((s: MunicipalityBudgetSnapshot) => s.area_name === areaName)
    .map((s: MunicipalityBudgetSnapshot) => ({ year: s.year, value: s.value_mnkr }))
    .sort((a, b) => a.year - b.year);

  // ── Cross-kommun comparison (% of total budget) ──────────────────────────
  ...
  return (
    <div className="sdt-page">
      ...
      {/* ── Section: Cross-kommun comparison ────────────────────────── */}
      <div
        style={{
          border: "1px solid var(--color-border)",
          borderTop: "none",
      ...
      >
        ...
        >
          % av total budget, senaste år · {kommunName} markerad
        </div>
