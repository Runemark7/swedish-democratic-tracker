...
export function RegionDetailPage() {
  ...
  if (isLoading) {
    return <p className="text-sm text-on-surface-variant py-16 text-center">Laddar regiondata…</p>;
  }
  if (error || !region) {
    return (
      <div className="py-8 text-center">
        <p className="text-sm text-red-500">Regionen hittades inte.</p>
        <NavLink to="/region" className="text-sm text-primary hover:underline mt-2 inline-block">
          ← Alla regioner
        </NavLink>
      </div>
    );
  ...
  const totalVotePct = region.electionResults.reduce((s, r) => s + r.votePct, 0);
      ...
      <div className="rounded-xl p-5 border" style={{ background: "var(--color-surface-lowest)", borderColor: "var(--color-surface-high)" }}>
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <p className="text-sm text-on-surface-variant">{region.capital} (residensstad)</p>
          </div>
          <span className="text-xs font-mono text-on-surface-variant bg-surface-high px-2 py-1 rounded">
            Regionkod {region.code}
          </span>
        </div>
        ...
        <div className="flex gap-6 pt-3 border-t" style={{ borderColor: "var(--color-surface-high)" }}>
          <StatBlock
            value={region.population.toLocaleString("sv-SE")}
            label="Invånare"
            small
          />
          <StatBlock value={region.totalMandates} label="Mandat" small />
          <StatBlock value={region.electionYear} label="Senaste val" small />
          <StatBlock value={region.governingParties.join("+")} label="Styre" small />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="rounded-xl overflow-hidden border" style={{ borderColor: "var(--color-surface-high)" }}>
          <table className="w-full text-sm">
            ...
            <tbody>
              {region.electionResults.map((r, i) => {
                const color = PARTY_COLORS[r.party]?.bg ?? "#9ca3af";
                const isGov = region.governingParties.includes(r.party);
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 text-right font-mono text-on-surface font-bold">
                      {r.mandates}
                    </td>
            ...
            <tfoot>
              <tr className="border-t text-[11px] font-semibold text-on-surface-variant" style={{ borderColor: "var(--color-surface-high)", background: "var(--color-surface-low)" }}>
                ...
                <td className="px-4 py-2 text-right font-mono">{region.totalMandates}</td>
