...
import { useRegionKpiRanking, useRegionList } from "@/hooks/useDemocracy";
import { useMediaQuery } from "@/hooks/useMediaQuery";
import { BarsWithMean } from "@/features/budget/components/BarsWithMean";
import { REGION_KPI_META } from "./regionKpiMeta";

const INITIAL_LIMIT = 21; // all 21 regions shown by default
  ...
  return (
    <div className="sdt-page">
      ...
      >
        ...
        >
          Råvärde ({kpiUnit.trim()}), senaste år · {regionName} markerad
        </div>
        ...
        {sorted.length === 0 ? (
          ...
          <>
            <BarsWithMean
              points={visible.map((e) => ({ region_code: e.region_code, pct: e.value }))}
              mean={mean}
              maxValue={maxValue}
              highlightCode={code}
              codeToName={new Map(rankEntries.map((e) => [e.region_code, e.name]))}
              isMobile={isMobile}
            />
            {!showAll && sorted.length > INITIAL_LIMIT && (
              ...
              >
                VISA FLER REGIONER ({sorted.length - INITIAL_LIMIT} ST)
              </button>
            )}
          </>
        )}
